...
class Tests(unittest.TestCase):
    def test_maze_create_cells(self):
        ...
        self.assertEqual(len(maze.cells), num_cols)
        self.assertEqual(len(maze.cells[0]), num_rows)

    def test_create_entrance_exit(self):
        maze = Maze(0, 0, 5, 5, 10, 10)
        self.assertFalse(maze.cells[0][0].top_wall)
        self.assertFalse(maze.cells[-1][-1].bottom_wall)
